...
const DetailLivescore = () => {
  ...
  const home = {
    ...
    lineup: match.lineups?.map((player) =>
      player.team_id === homeId ? player.player_name : ""
    ),
  };
  console.log("home id", homeId);
  console.log("home lineup", home.lineup);
  const awayId =
  match.participants?.[0].meta?.location === "home"
    ? match.participants?.[0]?.id
    : match.participants?.[1]?.id;
...
const away = {
  ...
  lineup: match.lineups?.map((player) =>
    player.team_id === awayId ? player.player_name : ""
  ),
};
          ...
          <div className="w-[90%] h-[75%] flex  md:flex-row ">
            ...
            <div className=" w-[40%] flex md:flex-col md:justify-start items-center">
              ...
              <div className="h-[15%] w-full  flex md:justify-center items-center">
                <h1 className="text-2xl uppercase text-pf-white font-bold text-bebas-font">
                  {home.name}
                </h1>
              </div>
            </div>
            ...
            <div className=" w-[20%] flex md:flex-col  md:justify-center items-center">
              <div className="w-full h-[70%] flex gap-5 md:justify-center md:items-center">
                <h1 className="font-bold text-pf-white text-8xl">
                  {home.score}
                </h1>
            ...
            <div className=" w-[40%] flex md:flex-col md:justify-end items-center">
              ...
              <div className="h-[15%] w-full flex md:justify-center items-center">
                <h1 className="text-2xl uppercase text-pf-white font-bold ">
                  {away.name}
                </h1>
              </div>
            </div>
  ...
  <div className="bg-red-500 w-full flex flex-row">
    <div className="flex flex-col" >{home.lineup.map(player => <p>{player}</p>)}</div>
    <div className="flex flex-col" >{away.lineup.map(player => <p>{player}</p>)}</div>
